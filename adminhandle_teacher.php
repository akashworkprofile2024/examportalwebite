    ...
    <div class="container-fluid">
        <div class="row">
            ...
            <main class="col-sm-9 ms-sm-auto col-lg-10 px-sm-4">
                ...
                <div class="h1 text-light text-center">Teacher Enrolled</div>  
...
if (isset($_POST['delete'])) {
    ...
    $delete_sql = "DELETE FROM teacherdetail WHERE uid = $teacher_id";
...
$storesql = "SELECT * FROM teacherdetail WHERE name LIKE '%$search_name%' AND dept LIKE '%$search_dept%'";
...
if ($runstore_sql->num_rows > 0) {
    while ($row = $runstore_sql->fetch_assoc()) {
        $teacherimage = base64_encode($row['image']);
        echo "<tr class='text-light h5 '>";
        echo "<td><img src='data:image/jpeg;base64,$teacherimage' class='rounded-circle instructor-avatar'></td>";
        echo "<td>" . $row['name'] . "</td>";
        ...
        echo "<td>" . $row['pass'] . "</td>";
        echo "<td>
                <form method='GET' action='adminteacher_update.php'>
                    <input type='hidden' name='teacher_id' value='" . $row['uid'] . "'>
    ...
    echo "<tr><td colspan='10' class='text-light'>No Teachers found</td></tr>";
}
